...
public class secuencia {
    ...
    public static void main(String[] args) {
        ...
        Functor sumaF = new Suma();
        int resultadoSuma = aplicar(secuencia, secuencia.size(), sumaF);
        System.out.println("Suma con functor: " + resultadoSuma);

        Functor multiF = new Multi();
    ...
    private static void ordenar(ArrayList<Integer> lista, int ini, int n) {
    ...
    private static int aplicar(ArrayList<Integer> lista, int tam, Functor f) {

        int res;

        if (tam == 1) {
            res = f.operar(lista.get(0));
        } else {
            int resultadoParcial = aplicar(lista, tam - 1, f);
            res = f.operar(resultadoParcial, lista.get(tam - 1));
        }
